...
import com.fuelware.app.fw_manager.adapters.ReceiptListAdapter;
...
public class ReceiptListActivity extends SuperActivity {
            ...
            public void accept(Map<Object, Object> map) throws Exception {
                try {
                    if (map.containsKey(RxBus.ADD_ACTION)) {
                        ...
                        int pos = (int) map.get(RxBus.POSITION);
                        if (object instanceof ReceiptModel){
                            list.set(pos, (ReceiptModel) object);
                        }
                    }
                    adapter.refresh();
                    showNoRecordsFound();
    ...
    private void setupRecycler() {
        ...
        adapter = new ReceiptListAdapter(list, this);
    ...
    private void getCashReceipts() {
        ...
        APIClient.getApiService().getReceiptsList(authkey, mode).enqueue(new Callback<ResponseBody>() {
            @Override
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                try {
                    if (response.isSuccessful()) {
                        ...
                        MLog.showToast(ReceiptListActivity.this, jsonObject.getString("message"));
                        list.remove(index);
                        adapter.refresh();
                        dialog.dismiss();
                    } else {
