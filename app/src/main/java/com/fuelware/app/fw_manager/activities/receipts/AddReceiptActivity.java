package com.fuelware.app.fw_manager.activities.receipts;
...
public class AddReceiptActivity extends SuperActivity {
    ...
    DatePickerDialog.OnDateSetListener dateListener = new DatePickerDialog.OnDateSetListener() {
        ...
        public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
            ...
            etChequeDate.setError(null);
            String dateString = year1 + "-" + month1 + "-" + day1;
            etChequeDate.setText(MyUtils.dateToString(AppConst.SERVER_DATE_FORMAT, AppConst.APP_DATE_FORMAT, dateString));
        }
    };
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        linlayChequeReceipt.setVisibility(View.GONE);
        if (receiptType.equals(Const.RECEIPT_CASH)) {
            setTitle("Add Cash Receipts");
        } else if (receiptType.equals(Const.RECEIPT_CREDIT)) {
            setTitle("Add Credit Card Receipts");
        } else if (receiptType.equals(Const.RECEIPT_DEBIT)) {
            setTitle("Add Debit Card Receipts");
        } else if (receiptType.equals(Const.RECEIPT_LOYALTY)) {
            setTitle("Add Loyalty Card Receipts");
        } else if (receiptType.equals(Const.RECEIPT_CHEQUE)) {
            setTitle("Add Cheque Receipts");
            linlayChequeReceipt.setVisibility(View.VISIBLE);
        }
    ...
    private void setEventListeners() {
        ...
        etChequeDate.setOnClickListener(v -> {
            showDatePicker();
        });
    ...
    private void showDatePicker() {
        ...
        oldDate.set(Calendar.DAY_OF_YEAR, calender.get(Calendar.DAY_OF_YEAR) - 90);
    ...
    private void addReceipt() {
        ...
        if (receiptType.equals(Const.RECEIPT_CASH)) {
            ...
            model.setCheque_date(MyUtils.dateToString(AppConst.APP_DATE_FORMAT, AppConst.SERVER_DATE_FORMAT, chequeDate));
            model.setCheque_number(chequeNo);
        }
...
//                        item.setBusiness(customer.getString("business"));
    ...
    private boolean isValid() {
        ...
        chequeDate = etChequeDate.getText().toString().trim();
        chequeNo = etChequeNumber.getText().toString().trim();
        ...
        if (selectedCustomer == null) {
            ...
            if (bankName.isEmpty()) {
                ...
                etBankName.setError("Bank name must be atleast 3 character.");
                etBankName.requestFocus();
                return false;
            } else if (branch.isEmpty()) {
                etBranch.setError("Enter bank branch name.");
                etBranch.requestFocus();
                return false;
            } else if (branch.length() < 3) {
                etBranch.setError("Branch must be atleast 3 character.");
                etBranch.requestFocus();
                return false;
            } else if (chequeNo.isEmpty()) {
                etChequeNumber.setError("Enter cheque date.");
                etChequeNumber.requestFocus();
                return false;
            } else if (chequeNo.length() != 6) {
                etChequeNumber.setError("Cheque Number be exact 6 digit long.");
                etChequeNumber.requestFocus();
                return false;
            }
